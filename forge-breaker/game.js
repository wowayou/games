...
const meta=loadMeta();
const S={phase:'menu',wave:1,ore:0,hp:100,maxHp:100,combo:0,heat:0,score:0,balls:[],enemies:[],sparks:[],oreBits:[],paddle:{x:W/2,w:120,target:W/2},shots:0,cleared:0,level:1,damage:1,speed:650,pierce:1,crit:.05,multiball:0,fire:0,ice:0,lightning:0,magnet:0,forge:{damage:0,paddle:0,ore:0,hp:0},fusion:{},boss:null,paused:false};
...
function reset(){const maxHp=100+meta.forge.hp*15;Object.assign(S,{phase:'playing',wave:1,ore:0,hp:maxHp,maxHp,combo:0,heat:0,score:0,balls:[],enemies:[],sparks:[],oreBits:[],shots:0,cleared:0,level:1,damage:1.0*(1+meta.forge.damage*.08),speed:650,pierce:1,crit:.05,multiball:1,fire:0,ice:0,lightning:0,magnet:0,forge:{...meta.forge},fusion:{},boss:null,paused:false});S.paddle.w=145*(1+meta.forge.paddle*.1);S.paddle.x=W/2;S.paddle.target=W/2;spawnWave();ui();}
...
function spawnWave(){if(S.wave%4===0){spawnBoss();return}S.enemies=[];S.cleared=0;const rows=Math.min(3+Math.floor(S.wave/2),7),cols=Math.min(7+Math.floor(S.wave/3),10);const gap=5, ew=Math.min(64,(W-100)/cols-gap), left=(W-(cols*(ew+gap)-gap))/2;for(let r=0;r<rows;r++)for(let c=0;c<cols;c++){if(Math.random()<Math.min(.12+S.wave*.025,.4)&&r>1)continue;const hp=1+Math.floor(S.wave*.3);S.enemies.push({x:left+c*(ew+gap),y:105+r*42,w:ew,h:30,hp,max:hp,row:r,kind:r===0&&S.wave>=3?'elite':'normal',frozen:0,burn:0,angle:Math.random()*TAU,dead:false});}}
...
var _relaunchT=0;function tick(dt){if(S.phase!=='playing'||S.paused)return;const p=S.paddle;if(!S.balls.length&&S.enemies.some(e=>!e.dead)){_relaunchT+=dt;if(_relaunchT>1.2){_relaunchT=0;launch()}}else{_relaunchT=0}p.x+=(p.target-p.x)*(1-Math.exp(-15*dt));for(const e of S.enemies){if(e.dead)continue;e.angle+=dt*2;e.frozen=Math.max(0,e.frozen-dt);e.burn=Math.max(0,e.burn-dt);if(e.burn>0)e.hp-=dt*S.damage*.2;e.y+=(15+S.wave*3)*(e.frozen?0.28:1)*dt;if(e.y+e.h>H-145){S.hp-=e.kind==='elite'?12:6;e.dead=true;S.combo=0;S.heat=Math.max(0,S.heat-20);burst(e.x+e.w/2,e.y,'#f22',20);audio.boom()}}
