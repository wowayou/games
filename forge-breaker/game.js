...
function spawnWave(){if(S.wave%4===0){spawnBoss();return}S.enemies=[];S.cleared=0;const rows=Math.min(2+Math.floor(S.wave/2),7),cols=Math.min(6+Math.floor(S.wave/3),10);const gap=5, ew=Math.min(64,(W-100)/cols-gap), left=(W-(cols*(ew+gap)-gap))/2;for(let r=0;r<rows;r++)for(let c=0;c<cols;c++){if(Math.random()<Math.min(.12+S.wave*.025,.4)&&r>1)continue;const hp=1+Math.floor(S.wave*.25);S.enemies.push({x:left+c*(ew+gap),y:105+r*42,w:ew,h:30,hp,max:hp,row:r,kind:r===0&&S.wave>=3?'elite':'normal',frozen:0,burn:0,angle:Math.random()*TAU,dead:false});}}
function launch(x=W/2){if(S.phase!=='playing'||S.paused)return; audio.init();const count=1+S.multiball;for(let i=0;i<count;i++){const spread=(i-(count-1)/2)*.13;S.balls.push({x,y:H-100,vx:Math.sin(spread)*S.speed,vy:-Math.cos(spread)*S.speed,r:8,damage:S.damage*(Math.random()<S.crit?2.2:1),pierce:S.pierce,kind:chooseBall(),trail:[]});}S.shots++;audio.tone(260,.12,'sawtooth',.05)}
...
function damageEnemy(e,b){if(e.dead)return;const heatMul=1+S.combo/45;let dmg=b.damage*heatMul*(e.kind==='elite'?.62:e.kind==='boss'?.5:1);if(S.fusion.nosferatu)S.hp=Math.min(S.maxHp,S.hp+.35);if(b.kind==='vampire')S.hp=Math.min(S.maxHp,S.hp+.8);e.hp-=dmg;e.burn=S.fire?1.4:0;e.frozen=S.ice?.7:0;if(S.fusion.melting){e.burn=1.8;e.frozen=.9}S.combo++;S.heat=Math.min(100,S.heat+6);S.score+=Math.round(dmg*10*(1+S.combo/20));burst(b.x,b.y,BALLS[b.kind]?.color||'#ffb52e',5);audio.hit();if(S.lightning&&Math.random()<.38)chain(e);if(e.hp<=0){e.dead=true;S.cleared++;if(S.fusion.brood)for(let i=0;i<3;i++)S.balls.push({x:e.x+e.w/2,y:e.y,vx:(Math.random()-.5)*S.speed,vy:-S.speed*.6,r:5,damage:S.damage*.6,pierce:1,kind:'ember',trail:[]});const gain=Math.round((1+Math.floor(Math.random()*3)+Math.floor(S.wave/3))*(e.kind==='boss'?9:e.kind==='elite'?3:1)*(1+S.forge.ore*.15));dropOre(e.x+e.w/2,e.y+e.h/2,gain);burst(e.x+e.w/2,e.y+e.h/2,'#ffb52e',16);audio.tone(300+S.combo*10,.08,'sine',.045)}}
...
var _relaunchT=0;function tick(dt){if(S.phase!=='playing'||S.paused)return;const p=S.paddle;if(!S.balls.length&&S.enemies.some(e=>!e.dead)){_relaunchT+=dt;if(_relaunchT>0.8){_relaunchT=0;launch()}}else{_relaunchT=0}p.x+=(p.target-p.x)*(1-Math.exp(-15*dt));for(const e of S.enemies){if(e.dead)continue;e.angle+=dt*2;e.frozen=Math.max(0,e.frozen-dt);e.burn=Math.max(0,e.burn-dt);if(e.burn>0)e.hp-=dt*S.damage*.2;e.y+=(10+S.wave*2.5)*(e.frozen?0.28:1)*dt;if(e.y+e.h>H-145){S.hp-=e.kind==='elite'?10:5;e.dead=true;S.combo=0;S.heat=Math.max(0,S.heat-20);burst(e.x+e.w/2,e.y,'#f22',20);audio.boom()}}
