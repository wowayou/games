...
const meta=loadMeta();
const S={phase:'menu',wave:1,ore:0,hp:100,maxHp:100,combo:0,heat:0,score:0,balls:[],enemies:[],sparks:[],oreBits:[],paddle:{x:W/2,w:120,target:W/2},shots:0,cleared:0,level:1,damage:1,speed:420,pierce:0,crit:.05,multiball:0,fire:0,ice:0,lightning:0,magnet:0,forge:{damage:0,paddle:0,ore:0,hp:0},fusion:{},boss:null,paused:false};
...
function reset(){const maxHp=100+meta.forge.hp*15;Object.assign(S,{phase:'playing',wave:1,ore:0,hp:maxHp,maxHp,combo:0,heat:0,score:0,balls:[],enemies:[],sparks:[],oreBits:[],shots:0,cleared:0,level:1,damage:1.5*(1+meta.forge.damage*.08),speed:420,pierce:0,crit:.05,multiball:0,fire:0,ice:0,lightning:0,magnet:0,forge:{...meta.forge},fusion:{},boss:null,paused:false});S.paddle.w=140*(1+meta.forge.paddle*.1);S.paddle.x=W/2;S.paddle.target=W/2;spawnWave();ui();}
function spawnBoss(){S.enemies=[];S.cleared=0;const hp=70+S.wave*20;S.enemies.push({x:W/2-130,y:135,w:260,h:74,hp,max:hp,row:0,kind:'boss',frozen:0,burn:0,angle:0,dead:false});for(let i=0;i<6;i++){const gh=8+S.wave*2;S.enemies.push({x:W/2-160+i*58,y:232,w:48,h:26,hp:gh,max:gh,row:1,kind:'normal',frozen:0,burn:0,angle:0,dead:false})}}
function spawnWave(){if(S.wave%4===0){spawnBoss();return}S.enemies=[];S.cleared=0;const rows=Math.min(3+Math.floor(S.wave/2),8),cols=Math.min(5+Math.floor(S.wave/3),11);const gap=5, ew=Math.min(64,(W-100)/cols-gap), left=(W-(cols*(ew+gap)-gap))/2;for(let r=0;r<rows;r++)for(let c=0;c<cols;c++){if(Math.random()<Math.min(.1+S.wave*.015,.3)&&r>1)continue;const hp=2+Math.floor(S.wave*.5)+r;S.enemies.push({x:left+c*(ew+gap),y:105+r*42,w:ew,h:30,hp,max:hp,row:r,kind:r===0&&S.wave>=3?'elite':'normal',frozen:0,burn:0,angle:Math.random()*TAU,dead:false});}}
